...
class TabelaUtentes(db: SQLiteDatabase) {
    ...
    fun cria() {
        db.execSQL("CREATE TABLE $NOME_TABELA ( ${BaseColumns._ID} INTEGER PRIMARY KEY AUTOINCREMENT, $CAMPO_NOME_UTENTE TEXT NOT NULL, $CAMPO_DATA_DE_NASCIMENTO TEXT NOT NULL, $CAMPO_SEXO TEXT NOT NULL, $CAMPO_SERVICO_INTERNAMENTO INTEGER, $CAMPO_ENFERMEIRO_RESPONSAVEL INTEGER, FOREIGN KEY ($CAMPO_SERVICO_INTERNAMENTO) REFERENCES ${TabelaCuidados.CAMPO_NOME_CUIDADO}, FOREIGN KEY ($CAMPO_ENFERMEIRO_RESPONSAVEL) REFERENCES ${TabelaEnfermeiros.CAMPO_NOME_ENFERMEIRO} )" )
    }
