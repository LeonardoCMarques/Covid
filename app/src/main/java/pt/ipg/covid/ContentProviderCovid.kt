 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun query(
        ...
        return when (getUriMatcher().match(uri)) {
            URI_ENFERMEIRO -> TabelaEnfermeiros(bd).query(
 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun getType(uri: Uri): String? {
        return when (getUriMatcher().match(uri)) {
            URI_ENFERMEIRO -> "$MULTIPLOS_ITEMS/$ENFERMEIRO"
            URI_UTENTE -> "$UNICO_ITEM/$UTENTE"
            URI_CUIDADO -> "$UNICO_ITEM/$CUIDADO"
            else -> null
        }
    }
 ...
 * and Threads](
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        ...
        val id = when (getUriMatcher().match(uri)) {
            URI_ENFERMEIRO -> TabelaEnfermeiros(bd).insert(values!!)
            URI_UTENTE -> TabelaUtentes(bd).insert(values!!)
            URI_CUIDADO -> TabelaCuidados(bd).insert(values!!)
            else -> -1L
        }
 ...
 * and Threads](
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        ...
        return when (getUriMatcher().match(uri)) {
            ...
            URI_CUIDADO_ESPECIFICO -> TabelaCuidados(bd).delete(
                "${BaseColumns._ID}=?",
                arrayOf(uri.lastPathSegment!!)
            )
 ...
 * and Threads](
    ...
    override fun update(
        ...
        return when (getUriMatcher().match(uri)) {
            ...
            URI_CUIDADO_ESPECIFICO -> TabelaCuidados(bd).update(
                values!!,
                "${BaseColumns._ID}=?",
                arrayOf(uri.lastPathSegment!!)
            )
    ...
    companion object {
        ...
        private const val CUIDADO = "cuidado"

        private const val URI_ENFERMEIRO = 100
        private const val URI_ENFERMEIRO_ESPECIFICO = 101
        private const val URI_UTENTE = 200
        private const val URI_UTENTE_ESPECIFICO = 201
        private const val URI_CUIDADO = 300
        private const val URI_CUIDADO_ESPECIFICO = 301
        ...
        private fun getUriMatcher() : UriMatcher {
            ...
            uriMatcher.addURI(AUTHORITY, ENFERMEIRO, URI_ENFERMEIRO)
            ...
            uriMatcher.addURI(AUTHORITY, CUIDADO, URI_CUIDADO)
            uriMatcher.addURI(AUTHORITY, "$CUIDADO/#", URI_CUIDADO_ESPECIFICO)
